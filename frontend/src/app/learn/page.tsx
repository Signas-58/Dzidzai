        ...
        <div className="card">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="text-xs rounded-full border border-gray-200 bg-gray-50 px-2.5 py-1 text-gray-700">AI-generated</div>
              <div className={`text-xs border rounded-full px-2.5 py-1 ${confUi.className}`}>{confUi.label}</div>
            </div>
          </div>

          <div className="mt-2 text-xs text-gray-500">
            This lesson was generated by AI. Please review for correctness.
          </div>
          ...
          {showFlag ? (
            <div className="mt-4 rounded-lg border border-gray-200 p-4">
              <div className="text-sm font-semibold text-gray-900">Quality flag</div>
              <div className="mt-1 text-xs text-gray-600">Report issues so we can improve safety and quality.</div>
              ...
              <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    <option value="Not age appropriate">Not age appropriate</option>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="mt-1 h-10 w-full rounded-md border border-gray-200 bg-white px-3 text-sm text-gray-900"
                    placeholder="e.g. Question 3 answer seems wrong"
                    disabled={flagBusy}
                  />
                </div>
              </div>
              ...
              <div className="mt-3 flex gap-2">
                <Button
                  ...
                  onClick={async () => {
                    if (!tokens?.accessToken) return;
                    if (!lastLessonKey) {
                      toast.error('Generate a lesson first');
                      return;
                    }
                    setFlagBusy(true);
                    try {
                      await apiFetch(`/content/flag`, {
                        method: 'POST',
                        token: tokens.accessToken,
                        body: {
                          lessonId: lastLessonKey,
                      ...
                      toast.error(e instanceof Error ? e.message : 'Failed to submit flag');
                    } finally {
                      setFlagBusy(false);
                    }
                  }}
